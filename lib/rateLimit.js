...
// General-purpose version of the above for throttling regular write
// actions (not just auth) — e.g. stopping a single creator from scripting
// hundreds of posts in a loop. Same underlying table/shape, just with
// caller-supplied window and limit instead of the auth-specific constants.
export async function checkAndRecordActionRate(identifier, windowMinutes, maxRequests) {
  const key = `action:${identifier}`;
  const { data } = await supabaseAdmin.from('auth_attempts').select('*').eq('identifier', key).single();
  const now = new Date();
  ...
  const windowStart = data?.updated_at ? new Date(data.updated_at) : null;
  const windowExpired = !windowStart || (now - windowStart) > windowMinutes * 60000;
  const nextCount = windowExpired ? 1 : (data?.attempts || 0) + 1;
  const locked_until = nextCount > maxRequests ? new Date(now.getTime() + windowMinutes * 60000).toISOString() : null;

  await supabaseAdmin.from('auth_attempts').upsert({ identifier: key, attempts: nextCount, locked_until, updated_at: now.toISOString() });
  if (locked_until) return { allowed: false, retryAfterSeconds: windowMinutes * 60 };
  return { allowed: true };
}
