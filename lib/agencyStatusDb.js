...
export async function getAgencyStatus(agencyId) {
  const { data } = await supabaseAdmin.from('agencies').select('status, trial_ends_at').eq('id', agencyId).single();
  return { status: data ? data.status : null, trialEndsAt: data ? data.trial_ends_at : null };
}
