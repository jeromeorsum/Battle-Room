...
export default async function handler(req, res) {
  ...
  let { data: agency } = await supabaseAdmin
    .from('agencies').select('id, name, plan_tier, billing_period, status, max_creators, trial_ends_at, referral_code, accent_color, stripe_current_period_end, stripe_cancel_at_period_end, contact_email, contact_email_verified').eq('id', session.agencyId).single();
